...
class Proxy2Log4jCheck(_BaseTestPoint):
    ...
    def _checkpoint(self):
        ...
        proxy_log4j = "/opt/oss/NSN-nbi3gc/proxy-2/etc/log4j.xml"
        ...
        if proxy_info_log_maxSize > conversion("50MB"):
            ...
            self.RCA.append("The maxFileSize of proxy-2 Info logs over limit. (> 50MB)")
            self.FIXSTEP.append("reset configuration %s Info logs maxFileSize to under 50MB."%proxy_log4j)
            self.logger.error("Proxy-2 info logs maxFileSize over limit, maxFileSize is %sbytes > 50MB" % proxy_info_log_maxSize)

        if proxy_trace_log_maxSize > conversion("100MB"):
            self.status = FAIL
            self.IMPACT.append("disk /var usage over limit for 3GPP Corba FM.")
            self.RCA.append("The maxFileSize of proxy-2 Trace logs over limit. (> 100MB)")
            self.FIXSTEP.append("reset configuration %s Trace logs maxFileSize to under 100MB." % proxy_log4j)
            self.logger.error("Proxy-2 trace logs maxFileSize over limit, maxFileSize is %sbytes > 100MB" % proxy_trace_log_maxSize)

        if proxy_error_log_maxSize > conversion("50MB"):
            self.status = FAIL
            self.IMPACT.append("disk /var usage over limit for 3GPP Corba FM.")
            self.RCA.append("The maxFileSize of proxy-2 Error logs over limit. (> 50MB)")
            self.FIXSTEP.append("reset configuration %s Error logs maxFileSize to under 50MB." % proxy_log4j)
            self.logger.error("Proxy-2 error logs maxFileSize over limit, maxFileSize is %sbytes > 50MB" % proxy_error_log_maxSize)
